...
!
!     ACT
!_A    Lois de paroi, modele k-w de Wilcox, parois adiabatiques
!_A    - la production de k est imposee a la paroi
!_A    - la dissipation omega est imposee a la paroi
!_A
!
...
!
    ...
    integer          ::   ntab
    double precision ::      ca,  cklb2,  cklb3,   cmu2,   cmu3
    double precision ::   coefa,  coefb,  coefc, coefc1,  coefp
    double precision ::    conv,   cste,    ctk,   ctmu,  ctmu2
    double precision ::   dconv,   dist,  dist2,   dnum,    dpc
    double precision ::    dpdt,   dpdx,   dpdy,   dpdz,   dtop
    double precision ::      dy,echleps,    f1i,   fgam,   fmui
    double precision ::     fxi,     ki,     li,     mu,    mup
    double precision ::     mut,    nxn,    nyn,    nzn,  omega
    double precision ::      ps,  retur,   rhoi,    rk2,   rok2
    double precision ::     rop,  seuil,     sn,   som1,   som2
    double precision ::    som3,     sv,     t1,     t2,     t3
    double precision ::    temp,     tn,    top,   top0,  topar
    double precision ::  topini,  tprod,     tt,  upyp1,   utau
    double precision ::       v,    v1t,    v1x,    v1y,    v1z
    double precision ::     vol,     xi,     yi,   yp02
    logical          :: lamin
!
!-----------------------------------------------------------------------
!
...
!       calcul echelle de longueur l
       v(ni,7)=cklb3*v(ni,1)*sqrt(0.5*v(ni,6)*v(ni,1))/v(ni,7)
!       distance
       dy=dist(ni)/(ndis-1)
!**************************************************************************
...
!        calcul de omega = k^0.5/cmu*l_eps en cellule 1
