...
contains
  subroutine partitionnement(x,y,z,ncbd,mnc,ncin,bceqt,exs1,exs2,nblocks,nsplit,nsplit_dir)
      ...
      double precision    :: vbc(ista*lsta),sub_bc_c(2,6),sub_bc2(5,3)
      ...
      if(.true.) then ! always do the partitionning in order to reequilibrate
        ...
        if(tab_raccord(frgi1)/=0) then ! if boundary is shared with another block
          ...
          if(rank==proci1) then
            ...
            !
            sub_bc2(5,:) = 1
            if(sub_bc(1,1)+1>=save_id2(lli1)) sub_bc2(5,1) = -1
            if(sub_bc(3,1)+1>=save_jd2(lli1)) sub_bc2(5,2) = -1
            if(sub_bc(5,1)+1>=save_kd2(lli1)) sub_bc2(5,3) = -1
            
            
            nbi = nb+sub_bc2(5,1)*1
            nbj = nb+sub_bc2(5,2)*nid
            nbk = nb+sub_bc2(5,3)*nijd
          ...
          if(rank==proci2) then
            ...
            na = save_npn(lli2)+1+(i1-save_id1(lli2)) &
                +(j1-save_jd1(lli2))*nid &
                +(k1-save_kd1(lli2))*nijd
            dist=sqrt( (save_x(na)-sub_bc2(2,1))**2+(save_y(na)-sub_bc2(2,2))**2+ &
                (save_z(na)-sub_bc2(2,3))**2 )
            if(dist<mindist) then
              mindist=dist
              minpos=na
            endif
            ...
            na = save_npn(lli2)+1+(i1-save_id1(lli2)) &
                +(j1-save_jd1(lli2))*nid &
                +(k1-save_kd1(lli2))*nijd
            dist=sqrt( (save_x(na)-sub_bc2(4,1))**2+(save_y(na)-sub_bc2(4,2))**2+ &
                (save_z(na)-sub_bc2(4,3))**2 )
            if(dist<mindist) then
              mindist=dist
              minpos=na
            endif
  ...
  subroutine num_split(nblock2,lt,nxyza,nblocks,ii2,jj2,kk2)
      ...
      where(nblock2>0) nblock2=-nblock2 ! block user values
        ...
        subroutine copy_grid(l,l2,xs,ys,zs,x,y,z,save_x,save_y,save_z,       &
            ...
            do zi=kk1(l2)-1,kk2(l2)+1
            do yi=jj1(l2)-1,jj2(l2)+1
            do xi=ii1(l2)-1,ii2(l2)+1
