...
!
    ...
    double precision ::    dtdx,   dtdy,   dtdz,   dvdv,   dvxx
    double precision ::    dvxy,   dvxz,   dvyx,   dvyy,   dvyz
    double precision ::    dvzx,   dvzy,   dvzz,     f1,     fd
    double precision ::   gkgro,   glgk,   glgl,  glgro,     mu
    double precision ::     mut, nlaplu,nutilde, qcxts5, qcyts6
    double precision ::    qsi2,   rack,  racrk,  racro, rdelta
    double precision ::   rtil3, rtilde,    sk2,    sl1,    sl2
    double precision ::     sl3,    sl4,    sl5,     sn,     ss
    double precision ::       t,    tn1,    tn2,    tn3,    tn9
...
!
    ...
    dimension mut(ip12),mu(ip12),dist(ip12),mnpar(ip12),txxf5x(ip12), &
         ...
         tzzf6z(ip12),qcxts5(ip12),qcyts6(ip12),vol(ip11)
    dimension cfke(ip13)
    dimension sn(ip31*ndir)
    dimension dvxx(ip00),dvxy(ip00),dvxz(ip00),dvyx(ip00),dvyy(ip00), &
         dvyz(ip00),dvzx(ip00),dvzy(ip00),dvzz(ip00), &
         dtdx(ip00),dtdy(ip00),dtdz(ip00),t(ip00),tprod(ip00), &
         bark(ip00),tn1(ip00),tn2(ip00),tn3(ip00),tn9(ip00)
